        ...
        if (!empty($detail)) {
            ...
            if (!empty($detail['min_price'])) {
                if ($total < $detail['min_price']) {
                    $alert['error'] = 'Mã ưu đãi áp dụng cho đơn hàng có giá trị tối thiểu là ' . str_replace(',', '.', number_format($detail['min_price'])) . '₫.';
                }
            }
            /*check giá trị đơn hàng tối đa*/
            if (!empty($detail['max_price'])) {
                if ($total > $detail['max_price']) {
                    $alert['error'] = 'Mã ưu đãi áp dụng cho đơn hàng có giá trị tối đa là ' . str_replace(',', '.', number_format($detail['max_price'])) . '₫.';
                }
            }
        ...
        if (!empty($coupon)) {
            foreach ($coupon as $v) {
                ...
                $html .= '<table><tr>
                    ...
                    <td>-<span class="amount cart-coupon-price">' . number_format($v['price']) . '₫</span> <a href="javascript:void(0)" data-id="' . $v['id'] . '" class="remove-coupon text-red-600 font-bold">[Xóa]</a></td>
                </tr> </table>';
            }
        }
