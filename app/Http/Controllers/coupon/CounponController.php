...
class CounponController extends Controller
{
    ...
    {
        $module = $this->table;
        $product_ids = \App\Models\Product::select('id', 'title')->where(['alanguage' => config('app.locale')])->get();
        $product_categories = \App\Models\CategoryProduct::select('id', 'title')->where(['alanguage' => config('app.locale')])->get();
        return view('coupon.create', compact('module', 'product_ids', 'product_categories'));
    }
