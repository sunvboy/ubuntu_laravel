    ...
    <div class="container px-4 mx-auto w-full flex flex-wrap items-center justify-between">
        <nav class="bg-grey-light w-full" aria-label="breadcrumb">
            <ol class="list-reset flex">
                ...
                <li><span class="text-gray-500 mx-2">/</span></li>
                <li><a href="javascript:void(0)" class="text-gray-500 hover:text-gray-600 text-f13">{{trans('index.UpdateOrder')}} #{{$detail->code}}</a></li>
            </ol>
        </nav>
    </div>
</nav>
    ...
    <div class="container px-4 mx-auto">
        <div class="mt-4 flex flex-col md:flex-row items-start md:space-x-4">
            ...
            <div class="flex-1 w-full md:w-auto order-1 md:order-1">
                <form action="{{route('customer.updateOrder',['id'=>$detail->id])}}" id="myFormUpdate" method="post">
                    @csrf
                    ...
                    <div class="shadowC rounded-xl p-6 space-y-10 md:space-y-4">
                        ...
                        <div class="flex flex-col space-y-2">
                            <div class="flex">
                                <input type="text" class="hidden js_provisional_input" value="<?php echo $detail->total_price ?>">
                        ...
                        <div class="">
                            <div>
                                ...
                                <div class="flex flex-col space-y-2 mt-3">
                                    ...
                                    <div class="flex">
                                        <input name="title_ship" type="text" class="hidden">
                                        <input name="fee_ship" type="text" class="hidden">
                                        <div class="w-1/2 md:w-3/4 text-right">
                                            {{trans('index.TransportFee')}}
                                        </div>
                                        <div class="flex-1 text-right font-bold js_fee_shipping">
                                ...
                                <div class="mt-5 flex justify-end">
                                    <button type="submit" class=" text-center leading-none uppercase bg-red-600 text-white text-sm bg-dark px-10 py-5 transition-all hover:bg-orange font-semibold">{{trans('index.UpdateOrder')}}</button>
                                    <button type="button" class=" text-center leading-none uppercase bg-red-600 text-white text-sm bg-dark px-10 py-5 transition-all hover:bg-orange font-semibold flex items-center hidden" disabled="">
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 200) {
                    ...
                    $("#myFormUpdate .print-error-msg").css('display', 'none');
                    toastr.success('<?php echo session('success') ?>', '<?php echo trans('index.OrderUpdateSuccessful') ?>')
                    setTimeout(function() {
                        location.reload();
                    }, 1500);
                } else {
                    $("#myFormUpdate .print-error-msg").css('display', 'block');
                    $("#myFormUpdate .print-success-msg").css('display', 'none');
                    $("#myFormUpdate .print-error-msg span").html(data.error);
                    $('html, body').animate({
                        scrollTop: $('#myFormUpdate').offset().top - 100
                    }, 100);
                    return false;
                }
            }
