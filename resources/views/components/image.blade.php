<div class="mt-3">
    ...
    <input type="text" value="<?php echo !empty($detail->$name) ? $detail->$name : ''; ?>" name="{{$name}}_old" class="form-control d-none" id="{{$name}}_old">
    <div class="d-flex mt-3">
        <div class="input-file-container w-50">
            <input class="d-none input-file w-100" id="my-{{$name}}" onchange="mainThamUrl(this,'{{$name}}')" name="{{$name}}" type="file">
            <label class="btn btn-dark w-100 input-file-trigger" for="my-{{$name}}">
                Upload file...
            </label>
        </div>
        <div class="uploadCkfinder w-50 ms-2">
            <label class="btn btn-dark w-100 input-file-trigger" onclick="selectFileUpload('{{$name}}');return false;">
                Select file
            </label>
        </div>
    </div>
