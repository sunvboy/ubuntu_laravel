...
<section class="bread-crumb" style="background: linear-gradient(0deg, rgba(0,0,0,0.8), rgba(0,0,0,0.3)),  url(//bizweb.dktcdn.net/100/485/131/themes/906771/assets/breadcrumb.jpg?1686556941849) no-repeat center;">
        <div class="container">
            ...
            <nav class="bg-grey-light w-full" aria-label="breadcrumb">
                  ...
                  @endforeach
                  <li><span class="text-gray-500 mx-2">/</span></li>
                  <li><a href="<?php echo route('routerURL', ['slug' => $detail->slug]) ?>" class="text-[#3bb77e] hover:text-gray-600  ">{{ $detail->title}}</a></li>
               </ol>
                </nav>
   ...
   <div class="grid md:grid-cols-12 gap-5">
      <div class="md:col-span-9">
         <div class="space-y-2">
            <h1 class="leading-8 text-f22 md:text-f25 font-semibold text-center">{{$detail->title}}</h1>
      ...
      <div class="md:col-span-3 space-y-8">
         ...
         <!-- @if(!$asideArticle->isEmpty())
            @foreach($asideArticle as $item)
            @if(count($item->posts) > 0)
            <div class="">
                <div class="bg-[#f0f0f0] p-[10px] rounded-[5px] mb-4">
                    <a href="{{route('routerURL',['slug' => $item->slug])}}" class="uppercase text-f18 font-bold">{{$item->title}}</a>
                </div>
                @foreach($item->posts as $k=>$val)
                @if($k == 0)
                <div class="group first overflow-hidden">
                    <a href="{{route('routerURL',['slug' => $val->slug])}}" class=" space-y-3 overflow-hidden">
                        <img data-src="{{asset($val->image)}}" alt="{{$val->title}}" class="lazy rounded mb-3">
                    </a>
                    <h3 class="text-f17 font-bold overflow-hidden leading-5 group-hover:text-[#3bb77e] hover:text-[#3bb77e]"><a href="{{route('routerURL',['slug' => $val->slug])}}">{{$val->title}}</a></h3>
                </div>
                @endif
                @endforeach
                <ul class="space-y-1 list-disc mt-3 pl-5">
                    ...
                    @if($k > 0)
                    <li>
                        <a href="{{route('routerURL',['slug' => $val->slug])}}" title="{{$val->title}}" class="hover:text-[#3bb77e]">{{$val->title}}</a>
                    </li>
                    @endif
                    @endforeach
                </ul>
                  ...
                  <div class="swiper-slide ">
                     <div class="hover-box ">
                        <div class=" img-box overflow-hidden">
                           ...
                           <img alt="{{$item->title}}" class="lazy w-full h-auto lg:h-full" src="{{asset($item->image)}}" style="display: block;">
                           </a>
                        </div>
                        <div class="nav-img">
                           <a href="{{route('routerURL',['slug' => $item->slug])}}" class="text-f15 md:text-f18 font-bold leading-6 clamp-2 ">{{$item->title}}</a>
                           <div class="content-nav">
                              <span class="text-f13 text-[#fff] italic">{{$detail->created_at}}</span>
